...
class ImageLabel(QtWidgets.QLabel):
    ...
    bboxFinished = Signal(float, float, float, float)
    wheelScrolled = Signal(int)  # 正数 / 负数 表示滚动方向
    ...
    def __init__(self, parent=None):
        ...
        self._rubber_band = QtWidgets.QRubberBand(
            QtWidgets.QRubberBand.Shape.Rectangle,  # Qt6 枚举
            self
        )
    ...
    def mousePressEvent(self, event: QtGui.QMouseEvent):
        ...
        if self.mode == "point":
            ...
            # 框选模式：记录起点，并显示橡皮筋
            self._origin = QtCore.QPoint(int(x), int(y))
